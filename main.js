...
// main.ts
var etymo = new Etymo();
var LanguagePromptModal = class extends import_obsidian3.Modal {
  constructor(app, selection, onSubmit) {
    super(app);
  ...
  onOpen() {
    ...
    contentEl.createEl("p", { text: "Search etymology / Buscar etimolog\xEDa / Rechercher l'\xE9tymologie" });
    ...
    const spanishButton = buttonContainer.createEl("button", { text: "Espa\xF1ol" });
    spanishButton.style.marginRight = "12px";
    spanishButton.addEventListener("click", () => {
      this.onSubmit("es");
      this.close();
    });
    const frenchButton = buttonContainer.createEl("button", { text: "Fran\xE7ais" });
  ...
  async onOpen() {
    ...
    try {
      ...
      if (this.lang === "en") {
        ...
        dpdText.setText(etymDPD != null ? etymDPD : "No se ha encontrado la etimolog\xEDa.");
        ...
        dleText.setText(etymDLE != null ? etymDLE : "No se ha encontrado la etimolog\xEDa.");
        const deChileHeader = contentEl.createEl("h3", { text: "Fuente: Diccionario Etimol\xF3gico de Chile" });
        const deChileText = contentEl.createEl("div");
        deChileText.style.whiteSpace = "pre-wrap";
        deChileText.setText(etymDeChile != null ? etymDeChile : "No se ha encontrado la etimolog\xEDa.");
        ...
        wiktionaryText.setText(etymWiktionary != null ? etymWiktionary : "Aucune \xE9tymologie trouv\xE9e.");
        ...
        cnrtlText.setText(etymCNRTL != null ? etymCNRTL : "Aucune \xE9tymologie trouv\xE9e.");
      }
    } catch (error) {
...
async function fetchSpanishEtymologyDPD(word) {
  var _a2, _b, _c;
  try {
    ...
    for (const section of sections) {
      ...
      if (header && ((_a2 = header.textContent) == null ? void 0 : _a2.trim().toLowerCase()) === "etimolog\xEDa") {
        const etimologyText = (_b = section.textContent) == null ? void 0 : _b.replace(/^etimología\s*/i, "").trim();
        if (etimologyText)
          return etimologyText;
      }
    }
    const firstSenseP = doc.querySelector("p[data-heading='sense']");
    if (firstSenseP) {
      const text3 = (_c = firstSenseP.textContent) == null ? void 0 : _c.trim();
      if (text3)
        return text3;
    }
    return null;
...
async function fetchSpanishEtymologyDLE(word) {
  var _a2;
  try {
    ...
    if (section) {
      ...
      if (etimDiv) {
        return ((_a2 = etimDiv.textContent) == null ? void 0 : _a2.trim()) || null;
      }
    }
    return null;
  } catch (error) {
...
async function fetchSpanishEtymologyDeChile(word) {
  var _a2, _b;
  try {
    ...
    const text3 = new TextDecoder("latin1").decode(new Uint8Array(buffer));
    const parser = new DOMParser();
    const doc = parser.parseFromString(text3, "text/html");
    const h3Elements = Array.from(doc.querySelectorAll("h3"));
    let targetH3 = null;
    for (const h3 of h3Elements) {
      if (((_a2 = h3.textContent) == null ? void 0 : _a2.trim().toLowerCase()) === word.toLowerCase()) {
        targetH3 = h3;
        break;
      }
    }
    ...
    while (sibling && sibling.tagName.toLowerCase() === "p") {
      const text4 = (_b = sibling.textContent) == null ? void 0 : _b.trim();
      if (text4)
        etymologyTexts.push(cleanText(normalizeText(text4)));
      sibling = sibling.nextElementSibling;
    }
    if (etymologyTexts.length === 0)
      return null;
...
async function fetchFrenchEtymologyWiktionary(word) {
  var _a2;
  try {
    ...
    const parser = new DOMParser();
    const doc = parser.parseFromString(response.text, "text/html");
    const etymologySection = doc.querySelector("#\xC9tymologie, #Etymologie");
    if (!etymologySection)
      return null;
    let etymologyText = "";
    let nextElement = etymologySection.nextElementSibling;
    while (nextElement && !["h2", "h3"].includes(nextElement.tagName.toLowerCase())) {
      const text3 = (_a2 = nextElement.textContent) == null ? void 0 : _a2.trim();
      if (text3)
        etymologyText += text3 + "\n";
...
async function fetchFrenchEtymologyCNRTL(word) {
  var _a2;
  try {
    ...
    while (currentElement && currentElement.id !== "contentbox") {
      const text3 = (_a2 = currentElement.textContent) == null ? void 0 : _a2.trim();
      if (text3)
        etymologyText += text3 + "\n";
...
function normalizeText(text3) {
  ...
  textarea.innerHTML = text3;
  let normalized = textarea.value;
  normalized = normalized.replace(/Ã¡/g, "\xE1").replace(/Ã©/g, "\xE9").replace(/Ã­/g, "\xED").replace(/Ã³/g, "\xF3").replace(/Ãº/g, "\xFA").replace(/Ãñ/g, "\xF1").replace(/Ã/g, "\xC1").replace(/Ã‰/g, "\xC9").replace(/Ã/g, "\xCD").replace(/Ã“/g, "\xD3").replace(/Ãš/g, "\xDA").replace(/Ã‘/g, "\xD1").replace(/Ã¢/g, "\xE2").replace(/Ãè/g, "\xE8").replace(/Ãê/g, "\xEA").replace(/Ãî/g, "\xEE").replace(/Ãô/g, "\xF4").replace(/Ã»/g, "\xFB").replace(/Ã§/g, "\xE7").replace(/Ã€/g, "\xC0").replace(/Ã‚/g, "\xC2").replace(/Ãˆ/g, "\xC8").replace(/ÃŠ/g, "\xCA").replace(/ÃŽ/g, "\xCE").replace(/Ã”/g, "\xD4").replace(/Ã›/g, "\xDB").replace(/Ã‡/g, "\xC7");
  return normalized;
}
function cleanText(text3) {
  return text3.replace(/\s+/g, " ").replace(/(\r?\n){2,}/g, "\n\n").trim();
}
var EtymologyLookupPlugin = class extends import_obsidian3.Plugin {
  ...
  async promptAndLookup(selection) {
    if (!selection)
      return;
    new LanguagePromptModal(this.app, selection, (lang) => {
...
function getCurrentSelectedText(app) {
  ...
  if (editor) {
    ...
    if (typeof selection2 === "string" && selection2.trim()) {
      return selection2.trim();
    }
  }
